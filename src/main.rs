...
/// The marker that says a home bazelrc is ours and may be replaced.
///
/// ⛔ WITHOUT THIS, setup-tbzl WOULD EAT A DEVELOPER'S OWN ~/.bazelrc. That file is not
/// hypothetical on this estate — a `--remote_cache` line in one split the cache and executor
/// legs of an RBE build and cost an afternoon, with an error naming the server's blobstore for
/// a client-side misroute. Overwriting it silently would be strictly worse than that bug.
const HOME_RC_MARKER: &str = "GENERATED BY setup-tbzl";
    ...
    if !args.no_home_bazelrc {
        ...
        match path {
            ...
            Some(p) => {
                ...
                // ⛔ NEVER CLOBBER A FILE WE DID NOT WRITE. A developer's own ~/.bazelrc is not
                // hypothetical on this estate — one carrying a --remote_cache line split the
                // cache and executor legs of an RBE build and cost an afternoon. Silently
                // deleting it would be strictly worse than that bug, so an unrecognised file is
                // a hard error naming the path and the escape hatch.
                let existing = std::fs::read_to_string(&p).ok();
                match existing {
                    Some(prev) if !prev.contains(HOME_RC_MARKER) => {
                        return Err(format!(
                            "{} already exists and was not written by setup-tbzl, so it has not \
                             been touched. It would be overwritten by the executor's layout \
                             (output base + shared caches). Move it aside, or pass \
                             --no-home-bazelrc and configure --output_user_root yourself — but \
                             note that a home rc is read AFTER the repo's .bazelrc and wins, so \
                             whatever is in that file is already overriding this repository",
                            p.display()
                        ))
                    }
                    _ => {
                        ...
                        println!(
                            "::notice title=setup-tbzl::wrote {} — every bazel invocation in this \
                             job now shares one output base and the host's caches, with no \
                             per-step flags",
                            p.display()
                        );
                    }
